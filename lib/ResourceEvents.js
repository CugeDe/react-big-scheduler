                ...
                if (headerItem.count > 0 || headerItem.summary != undefined) {
                    ...
                    headerItem.events.forEach(function (evt, idx) {
                        if (idx < renderEventsMaxIndex && evt !== undefined && evt.render) {
                            ...
                            if (cellUnit === _index.CellUnits.Day) {
                                var startDay = localeMoment(eventStart).startOf('day');
                                var diffStartDay = eventStart.diff(startDay, 'minutes');
                                var eventDurationMinutes = eventEnd.diff(eventStart, 'minutes');
                                // console.log('event id', evt.eventItem.id, diffStartDay, eventDurationMinutes);
                                _left += cellWidth / 1440 * diffStartDay;
                                if (_left - 2 > 0) _left -= 2;

                                if (isEnd) {
                                    _width = cellWidth / 1440 * eventDurationMinutes;
                                } else {
                                    _width -= cellWidth / 1440 * diffStartDay;
                                }
